...
Boss 會掉**星核** —— 6 系 × 5 階（碎片／凝聚／輝耀／燦爛／奇點）：

烈焰（攻擊）、磐石（防禦）、疾風（攻速）、銳目（暴擊）、血月（吸血）、生命（生命）
...
兩位玩家各出 3 名英雄，6 人出現在**同一場、同一條時間軸**的戰鬥裡合力打共鬥王。

- **雲端房間**（推薦）：開房拿 6 碼房碼給朋友，兩人共同磨一條王血池
  （雙人血池 ×1.7 次線性縮放），各自出戰、傷害即時累積，看得到彼此貢獻，
  也能重播隊友的出戰。房間 7 天有效；角色資料不上雲，只同步共鬥進度。
- **碼交換模式**（零網路依賴）：挑戰碼 → 戰報碼手動交換，同樣能重播出一致的戰鬥。
- 技術核心：戰鬥重播完全 **deterministic** —— 固定步長（10 tick/秒）＋種子亂數
  （mulberry32）＋全整數運算，重播路徑零浮點誤差來源；
  同一份輸入在 Node / Chrome / Edge 上 1000 次重跑雜湊完全一致。
- 後端：單一 Cloudflare Worker + KV（`server/`），只存最小共享狀態。
...
- **線上遊玩（分享用）**：https://dijkstra1115.github.io/pocket-abyss/
- 雙擊 **`啟動遊戲.bat`**：以 Edge 應用模式開一個無邊框小窗（推薦本機使用）。
- 或直接用瀏覽器開 `index.html`。

存檔存在各自瀏覽器的 localStorage：線上版與本機版進度互相獨立，
可用「設定 → 匯出/匯入存檔」搬移進度。
